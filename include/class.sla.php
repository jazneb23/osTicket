...
implements TemplateVariable {
    ...
    function addGracePeriod(Datetime $date, BusinessHoursSchedule $schedule
        ...
        $calculator = new SlaGracePeriodCalculator();
        return $calculator->addGracePeriod($date, $this->getGracePeriod(),
                $schedule, $timeline);
    }
    ...
    function isTransient() {
        $checker = new SlaTransientChecker();
        return $checker->check($this->flags, self::FLAG_TRANSIENT);
    }
    ...
    function priorityEscalation() {
        $resolver = new SlaPriorityEscalationResolver();
        return $resolver->resolve($this->flags, self::FLAG_ESCALATE);
    }
